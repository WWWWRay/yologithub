...
def train():
    ...
    # Configure run
    data_dict = parse_data_cfg(data)#读取data文件
    ...
    temp_look = dict(model.named_parameters())#读取dict
    ...
    if weights.endswith('.pt'):  # pytorch format
        ...
        load_darknet_weights(model, weights)#将权重加载到模型当中
    ...
    lf = lambda x: (1 + math.cos(x * math.pi / epochs)) / 2  #设置学习率下降的策略
    # 注意这里的epochs为之前的设置的总的epoch数,x代表当前epoch
    ...
    dataloader = torch.utils.data.DataLoader(dataset,
                                             ...
                                             pin_memory=True,
                                             collate_fn=dataset.collate_fn)#https://mp.weixin.qq.com/s/Uc2LYM6tIOY8KyxB7aQrOw
    ...
    maps = np.zeros(nc)  # 初始化class和results
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        mloss = torch.zeros(4).to(device)  # 计算损失
        print(('\n' + '%10s' * 8) % ('Epoch', 'gpu_mem', 'GIoU', 'obj', 'cls', 'total', 'targets', 'img_size'))
        pbar = tqdm(enumerate(dataloader), total=nb)  # progress bar
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            # targets shape[39,6] batch class x y w h
            ni = i + nb * epoch  # number integrated batches (since train start)
            imgs = imgs.to(device).float() / 255.0  #把数据送到GPU   uint8 to float32, 0 - 255 to 0.0 - 1.0
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data', type=str, default='dataft/ft.data.txt', help='*.data path')
