...
def create_modules(module_defs, img_size):
    ...
    output_filters = [int(hyperparams['channels'])]#读取channels
    module_list = nn.ModuleList()#创建一个list，其中存放的是module
    routs = []  # list of layers which rout to deeper layers
    yolo_index = -1
    """
    构建网络，配置kernel_size，padding等参数，根据不同的层进行不同的设计
    """
    for i, mdef in enumerate(module_defs):
        modules = nn.Sequential()#一个时序容器。Modules 会以他们传入的顺序被添加到容器中。
        # if i == 0:
        ...
        if mdef['type'] == 'convolutional':
            ...
            modules.add_module('Conv2d', nn.Conv2d(in_channels=output_filters[-1],
                                                   ...
                                                   bias=not bn))
            """
            在卷积神经网络的卷积层之后总会添加BatchNorm2d进行数据的归一化处理，这使得数据在进行Relu之前不会因为数据过大
            而导致网络性能的不稳定.
            """
            if bn:
            ...
            if mdef['activation'] == 'leaky':  # 激活函数的选择
        ...
        elif mdef['type'] == 'route':  # nn.Sequential() placeholder for 'route' layer，route里面没有Sequential
            layers = mdef['layers']
            filters = sum([output_filters[l + 1 if l > 0 else l] for l in layers])#取得route的输出通道
    ...
    routs_binary = [False] * (i + 1)#返回route的位置
    for i in routs:
        routs_binary[i] = True
...
class Darknet(nn.Module):
    ...
    def __init__(self, cfg, img_size=(416, 416)):
        ...
        self.module_defs = parse_model_cfg(cfg)#对cfg进行解析
    ...
    def forward(self, x, verbose=False):
        ...
        for i, (mdef, module) in enumerate(zip(self.module_defs, self.module_list)):
            ...
            out.append(x if self.routs[i] else [])#判断是否为route
            if verbose:
