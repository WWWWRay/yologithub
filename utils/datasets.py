...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=416, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        with open(path, 'r') as f:
            self.img_files = [x.replace('/', os.sep) for x in f.read().splitlines()  # 存放文件路径
        ...
        self.label_files = [x.replace('Images', 'txt').replace(os.path.splitext(x)[-1], '.txt')
                            for x in self.img_files]     #得到txt文件的路径
    ...
    #     return self
    """
    __getitem__()每次只处理一张图片,batch_size为16
    """
    def __getitem__(self, index):
        ...
        if self.mosaic:
            ...
            shapes = None#神经网络的数据必须是416*416的
        ...
        labels_out = torch.zeros((nL, 6))#此时labels是nadarray形式的将其转化为tensor才能输入神经网络
        if nL:
    ...
    def collate_fn(batch):
        img, label, path, shapes = zip(*batch)  # 解压
        for i, l in enumerate(label):
            l[:, 0] = i  # add target image index for build_targets()
        return torch.stack(img, 0), torch.cat(label, 0), path, shapes
    """
    stack（tensors,dim=0,out=None）函数的功能：将若干个张量在dim维度上连接,生成一个扩维的张量，
    比如说原来你有若干个2维张量，连接可以得到一个3维的张量。将img在一维上进行拼接
...
def load_mosaic(self, index):
    ...
    labels4 = []
    s = self.img_size#图片尺寸
    xc, yc = [int(random.uniform(s * 0.5, s * 1.5)) for _ in range(2)]  # mosaic center x, y得到中心
    img4 = np.zeros((s * 2, s * 2, 3), dtype=np.uint8) + 128  # base image with 4 tiles
    indices = [index] + [random.randint(0, len(self.labels) - 1) for _ in range(3)]  # 随机取另外三张图
    ...
    if len(labels4):
        ...
        np.clip(labels4[:, 1:], 0, 2 * s, out=labels4[:, 1:])  # 限制在0和832之间
